...
class Entity {
    ...
    constructor( worldCtrl = null, dom$ = null, staticBody = false, density = null, friction = null, restitution = null ) {
    	...
    	this.dom$ = dom$;           // The jQuery DOM element se have selected "actors"
    	this.physicsModel = null;   // The Box2D Rigid Body "model" we are managing
    	this.density = density;
    	this.frinction = friction;
    	this.restitution = restitution;
    	this.bodyDef = new Physics.BodyDef;  
    	
    	if(this.dom$.hasClass("bird-anim") || this.dom$.hasClass("item-cannonball"))
    	{
    ...
    __createModel( staticBody, density, friction, restitution ) {
        ...
        this.bodyDef.position.x = __m.mX / WORLD_SCALE;
        this.bodyDef.position.y = __m.mY / WORLD_SCALE;
    
        let fixDef = new Physics.FixtureDef;
        
        if(this.bodyType == "Circle")
        {
        	if(nav.level.levelProjectiles == 1 && this.bodyDef.awake)
        	{
        		console.log('sdas');
        	}
        ...
        fixDef.density = this.density;      // density * area = mass 
        fixDef.friction = this.frinction;     // 1 = sticky, 0 = slippery
        fixDef.restitution = this.restitution;  // 1 = very bouncy, 0 = almost no bounce
        
        let myWorldModel = this.world.model; 
        let aBodyModel = myWorldModel.CreateBody( this.bodyDef );
